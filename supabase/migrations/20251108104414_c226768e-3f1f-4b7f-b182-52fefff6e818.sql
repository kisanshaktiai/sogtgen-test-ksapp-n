-- Drop the old function that incorrectly tried to use geometry types
DROP FUNCTION IF EXISTS public.insert_land_with_geometry(UUID, UUID, TEXT, TEXT, NUMERIC, TEXT, UUID, TEXT, UUID, TEXT, UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, DATE, NUMERIC, NUMERIC, TEXT, TEXT);

-- Create corrected function to insert land with GeoJSON (JSONB) format
CREATE OR REPLACE FUNCTION public.insert_land_with_geometry(
  ...
  p_center_geojson JSONB DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Insert land record with GeoJSON stored as JSONB
  INSERT INTO public.lands (
    ...
    p_boundary_geojson,
    p_center_geojson,
    CASE WHEN p_boundary_geojson IS NOT NULL THEN 'gps_points' ELSE NULL END,
    CASE WHEN p_boundary_geojson IS NOT NULL THEN 10 ELSE NULL END,
    CASE WHEN p_boundary_geojson IS NOT NULL THEN NOW() ELSE NULL END,
    true,
    NOW(),
    NOW()
  )
