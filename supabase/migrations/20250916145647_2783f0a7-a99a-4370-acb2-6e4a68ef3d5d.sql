...
-- Create helper function to get farmer_id from JWT (update if exists)
...
WITH CHECK (
  -- For farmers, ensure they maintain their own IDs
  (
    farmer_id = get_jwt_farmer_id() 
    AND tenant_id = get_jwt_tenant_id()
  )
...
-- Update any existing community with global-like names to be global
UPDATE public.communities
SET is_global = true
WHERE LOWER(name) LIKE '%global%' 
   OR LOWER(name) LIKE '%all%farmer%'
