-- Add RLS policy for marketplace_categories
CREATE POLICY "Categories are viewable by everyone" ON marketplace_categories FOR SELECT USING (true);

-- Now create the marketplace tables with proper relationships
...
CREATE POLICY "Chat participants can view messages" ON marketplace_chat_messages FOR SELECT USING (EXISTS (SELECT 1 FROM marketplace_chats WHERE id = marketplace_chat_messages.chat_id AND (buyer_id = auth.uid() OR seller_id = auth.uid())));
CREATE POLICY "Chat participants can send messages" ON marketplace_chat_messages FOR INSERT WITH CHECK (auth.uid() = sender_id);

CREATE POLICY "Users can manage their search history" ON search_history FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Anyone can track views" ON product_views FOR INSERT WITH CHECK (true);

-- Function to update seller rating
CREATE OR REPLACE FUNCTION update_seller_rating() RETURNS TRIGGER 
SECURITY DEFINER
