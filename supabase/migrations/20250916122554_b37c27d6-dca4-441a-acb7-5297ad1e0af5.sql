...
-- Add is_global and community_type flag to communities table
ALTER TABLE public.communities 
ADD COLUMN IF NOT EXISTS is_global BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES public.tenants(id),
ADD COLUMN IF NOT EXISTS community_type TEXT DEFAULT 'tenant' CHECK (community_type IN ('global', 'tenant', 'crop', 'state', 'practice'));

-- Create global community if it doesn't exist
INSERT INTO public.communities (id, name, slug, description, community_type, is_global, is_active)
VALUES (
