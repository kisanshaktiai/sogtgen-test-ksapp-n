-- Add local_name column to crops table if it doesn't exist
...
-- Update duration with accurate growing periods
UPDATE crops SET duration = CASE
  -- Short duration (60-90 days)
  WHEN value IN ('spinach', 'radish', 'carrot', 'cucumber', 'okra', 'bottle_gourd') THEN '60-90 days'
  
  -- Medium duration (90-120 days)
  WHEN value IN ('rice', 'wheat', 'maize', 'tomato', 'onion', 'potato', 'chili', 'groundnut', 'moong', 'urad') THEN '90-120 days'
  
  -- Long duration (120-150 days)
  WHEN value IN ('cotton', 'tur', 'gram', 'mustard', 'sunflower', 'soybean') THEN '120-150 days'
  
  -- Very long duration (150+ days)
  WHEN value IN ('sugarcane', 'banana', 'coconut', 'mango', 'cashew') THEN '150+ days'
...
INSERT INTO crops (value, label, season, duration, local_name, is_popular, metadata, group_id)
SELECT * FROM (VALUES
  -- Additional Oilseeds
  ('castor', 'Castor', 'kharif', '150-180 days', 'अरंडी', false, '{"water_requirement": "low"}', 
   (SELECT id FROM crop_groups WHERE group_key = 'oilseeds' LIMIT 1)),
  ('safflower', 'Safflower', 'rabi', '120-150 days', 'कुसुम', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'oilseeds' LIMIT 1)),
  ('niger', 'Niger', 'kharif', '90-120 days', 'रामतिल', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'oilseeds' LIMIT 1)),
   
  -- Additional Pulses
  ('rajma', 'Kidney Beans', 'kharif', '90-120 days', 'राजमा', false, '{"water_requirement": "medium"}',
   (SELECT id FROM crop_groups WHERE group_key = 'pulses' LIMIT 1)),
  ('kulthi', 'Horse Gram', 'kharif', '90-120 days', 'कुलथी', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'pulses' LIMIT 1)),
  ('moth', 'Moth Bean', 'kharif', '60-90 days', 'मोठ', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'pulses' LIMIT 1)),
   
  -- Additional Vegetables
  ('garlic', 'Garlic', 'rabi', '120-150 days', 'लहसुन', true, '{"water_requirement": "medium"}',
   (SELECT id FROM crop_groups WHERE group_key = 'vegetables' LIMIT 1)),
  ('ginger', 'Ginger', 'kharif', '180-210 days', 'अदरक', true, '{"water_requirement": "high"}',
   (SELECT id FROM crop_groups WHERE group_key = 'vegetables' LIMIT 1)),
  ('turmeric', 'Turmeric', 'kharif', '210-240 days', 'हल्दी', true, '{"water_requirement": "high"}',
   (SELECT id FROM crop_groups WHERE group_key = 'vegetables' LIMIT 1)),
  ('coriander', 'Coriander', 'rabi', '90-120 days', 'धनिया', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'vegetables' LIMIT 1)),
  ('fenugreek', 'Fenugreek', 'rabi', '90-120 days', 'मेथी', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'vegetables' LIMIT 1)),
   
  -- Additional Grains/Millets
  ('ragi', 'Finger Millet', 'kharif', '90-120 days', 'रागी', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'grains' LIMIT 1)),
  ('kodo', 'Kodo Millet', 'kharif', '90-120 days', 'कोदो', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'grains' LIMIT 1)),
  ('kutki', 'Little Millet', 'kharif', '60-90 days', 'कुटकी', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'grains' LIMIT 1)),
  ('sanwa', 'Barnyard Millet', 'kharif', '60-90 days', 'सांवा', false, '{"water_requirement": "low"}',
   (SELECT id FROM crop_groups WHERE group_key = 'grains' LIMIT 1))
) AS new_crops(value, label, season, duration, local_name, is_popular, metadata, group_id)
WHERE NOT EXISTS (
